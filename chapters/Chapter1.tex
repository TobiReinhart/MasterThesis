...
Note that, as the two intertwiners are vector bundle morphisms, they can be understood as certain tensors themselves: 
\begin{align}
\begin{aligned}
...
Note that when utilizing this intertwiner technique, it is not necessary to specify explicit values of the two intertwiner maps. As long as they suffice the required properties all results that we are going to compute in this framework will be independent of the specific choice of intertwiners. Nevertheless, for concrete computations, it is often advantageous to stipulate the explicit values of the intertwiners, especially later on, when we are going to employ computer algebra. 
...
\begin{align}
    ...
    \end{aligned}
\end{align}
This approach can, for instance, be used if the tensor field at hand, $G \in \Gamma(F)$, is defined by requiring it to obey a specific symmetry\footnote{Symmetries that are defined in the following way cover most of the cases of symmetric tensor fields in classical field theory and certainly all cases that we will encounter in this thesis.}. More precisely, one requires that a certain subgroup of the direct product of permutation groups, $\operatorname{Sym}(G) \subset S_m \times S_n$, acts on its tensor indices by multiplication with a sign, i.e., it holds that
\begin{align}
    \text{for all } (\rho, \tau) \in \operatorname{Sym}(G) : G^{a_{\rho(1)}...a_{\rho(m)}}_{b_{\tau(1)}...b_{\tau(n)}} = \pm G^{a_1...a_m}_{b_1...b_n}.
\end{align}
We use these symmetries to define an equivalence relation on the set of $4^{m+n}$ fiber coordinate functions $\left \{ v^{a_1 ... a_m}_{b_1, ..., b_n} \ \big \vert \  a_1,...,a_m,b_1...b_n \in \{0,...,3 \} \right \}$ on $T^m_nM$. We call two such fiber coordinates $v^{a_1 ... a_m}_{b_1 ... b_n}$ and $v^{c_1 ... c_m}_{d_1 ... d_n}$ equivalent if once they are restricted to $F \subset T^m_nM$ they only differ by a sign:
...
\begin{align}\label{defI}
    ...
    \end{cases}
\end{align}
$J^{b_1 ... b_n}_{A a_1 ... a_m}$ is then defined such that the pair of intertwiners satisfies the last condition in (\ref{interRel}), i.e., $J$ is chosen as an left inverse of $I$. One readily finds that
...
In the end, we want to describe the gravitational field as sections of such a field bundle and construct for it a Lagrangian and the corresponding equations of motion. For that reason, it is necessary to introduce a notion of \textit{\textbf{differentiating}} such sections. 

It is crucial to keep in mind that one of the aims of the Constructive Gravity approach is allowing for a deviation from metric gravity theories. Hence, we are not dealing with spacetimes where the usual metric structures are present. In particular, there is, a priori, no distinguished connection on the frame bundle and therefore no covariant derivative that can be used to define derivatives of tensor fields in a coordinate-independent fashion. Note, however, that already in standard General Relativity those notions are strictly speaking not necessary. The Riemann tensor, one of the fundamental objects that are used to encode the dynamics that govern General Relativity, might be interpreted as curvature 2-form associated to the metric compatible connection on the frame bundle. 
In coordinates, however, it can be entirely specified in terms of the metric tensor, its inverse, and up to second partial derivatives of the corresponding component functions, without ever mentioning any deeper structure that these expressions might define. The same holds for the Einstein-Hilbert-Lagrangian. 
...
\begin{align}
    ...
    \end{aligned}
\end{align}
The corresponding equivalence class of a section $G$ at $p$ is then called the 1-jet of $G$ at $p$ and is denoted by $j^1_pG$.
\end{definition}
...
\end{remark}
Given adapted coordinates $(x^m,v_A)$ on $F$ we get induced adapted coordinates $(x^m, v_A, v_{Ai})$ on $J^1F$ by specifying the new $4 \cdot r$ coordinate functions as
\begin{align}
...
We have specified $4^2\cdot r$ new coordinate functions $v_{Aij}$ to construct adapted coordinates on $J^2F$. Due to the commutative property of the partial derivatives that are involved in the definition the $v_{Aij}$ actually only constitute $4\cdot (4+1)/2 \cdot r = 10\cdot r$ independent functions. To cope with such problems, one usually introduces multi-indices for the description of higher-order derivative coordinates on higher-order jet bundles. We will follow a slightly different approach by making use of the same intertwiner technique that we already have developed before, for the description of fiber coordinates on $F$. By doing so, it is easier for us to relate the derivative coordinates $v_{Aij}$ and possible higher jet coordinates to partial derivatives of sections in standard physics notation. 
Thus, for the description of second-order spacetime derivatives we construct intertwiners for the bundle $S_2M \subset T^0_2M$.

We proceed along the same lines as before when we constructed a pair of intertwiners for the bundle of metric tensor fields, i.e., we provide a pair of intertwiners and thereby define $10$ new indices that label second-order derivative in terms of the $16$ possible pairs of spacetime indices:
\begin{align}
    \begin{aligned}
...
Note that these are now intertwiners for the bundle $S_kM \subset T^0_kM$.
They can, in fact, be obtained entirely analog to the above example of $k=2$, we will, however, never need their explicit expression. 
...
To further account for such additional symmetries, we need to combine the two sets of spacetime indices in a new derivative index of order $k+m$. This suggests the definition of an additional intertwiner map that relates a derivative index of order $k+m$ with a pair of derivative indices of order $k$ and $m$. Given any object $u_{I_{k}}$ that carries such derivative indices ---for instance, the higher-order total derivative $D_{I_k}$--- we introduce
\begin{align}
   u_{I_{k+m}} := u_{I_k} u_{I_m}
...
\end{align}
Note that the variational derivative of a function on the $q$th-order jet bundle in general yields a function on the $2q$th-order jet bundle.
...
\end{figure}
Note that by giving a proper definition of the Lagrangian as a bundle map between $J^2F$ and $\Lambda^4M$ we are able to fully encode the properties of the corresponding action functional in terms of a smooth map between finite-dimensional manifolds. At no place, we actually have to work over infinite-dimensional spaces of sections and functionals defined on them. In particular, when working with the Lagrangian $\mathcal{L}$ we can safely apply the standard techniques of differential geometry. 
...
Note that this requirement is indispensable if the theory of gravity shall allow for a coordinate-independent formulation without referring to any, a priori specified background\footnote{Note that formulating a theory of gravity w.r.t.\ an additional, by hand specified background structure/geometry would dispossess it of its entire meaning, as one of the primary purposes of gravitational physics is to provide equations that determine the geometric structure of spacetime without having to postulate it.} structure other than that of a bare, differential manifold.
...
Let $(F_1,\pi_{F_1},M)$ and $(F_2,\pi_{F_2},N)$ be bundles, $\phi : M \rightarrow N$ a diffeomorphism, $f : F_1 \rightarrow F_2$ a bundle morphism covering $\phi$ and $J^1F_1$, $J^1F_2$ the first-order jet bundles over $F_1$ and $F_2$, respectively. The first jet bundle lift or prolongation of $(f,\phi)$ is given by the unique bundle morphism\footnote{Just as it is the case with bundle morphisms being denoted by pairs $(f,\phi)$ their prolongation should be denoted by triples $(j^1(f),f,\phi)$. However, we will lighten their notation to simply $j^1(f)$ if the two remaining maps are clear from the given context.} $j^1(f)$ that lets the diagram in figure \ref{ProlongMorph} commute.
\begin{figure}[hbt!]
\centering
\begin{tikzpicture}
\node (M) at (0,0) {$M$};
...
\begin{align}
\mathcal{f}\bigl ( \bigl [\xi, \chi \bigr ]_{\Gamma(TM)}\bigr ) = \bigl [ \mathcal{f}\bigl (\xi\bigr ), \mathcal{f}\bigl (\chi\bigr ) \bigr]_{\Gamma(TF)}.
\end{align}
...
\end{multline}
In precisely the same way one can now construct prolongations of vector fields to higher-order jet bundles, for instance, to the second-order jet bundle as it is relevant for our treatment of Lagrangian field theory:
\begin{align}
    \begin{aligned}
...
\begin{align}
j^q (\mathcal{f})\bigl (  \bigl [\xi, \chi  \bigr ]_{\Gamma(TM)}\bigr) =  \bigl [ j^q(\mathcal{f})(\xi), j^q(\mathcal{f})(\chi) \bigr ]_{\Gamma(TJ^qF)}.
\end{align}
...
\begin{aligned}
    0 &= f^{\tilde{C}}_{:m} \\
    0 &= f^{\tilde{C}:A} C_{An}^{Bm} v_B + f^{\tilde{C}:Ap} \bigl[ C_{An}^{Bm} \delta_p^q - \delta_A^B \delta^q_n \delta^m_p \bigr] v_{Bq} + f^{\tilde{C}:AI} \bigl[ C_{An}^{Bm} \delta_I^J - 2 \delta_A^B J_I^{pm} I^J_{pn}  \bigr] v_{BJ} - f^{\tilde{A}}K_{\tilde{A}n}^{\tilde{C}m}\\
...
From the diffeomorphism \textit{\textbf{equivariance equations}}, we easily see that in order to simplify the task of solving the system, we might proceed as follows:
...
\begin{align}
    0 &= L_{scal}^{:A} C_{An}^{Bm} v_B + L_{scal}^{:Ap} \bigl[ C_{An}^{Bm} \delta_p^q - \delta_A^B \delta^q_n \delta^m_p  \bigr] v_{Bq} + L_{scal}^{:AI} \bigl[ C_{An}^{Bm} \delta_I^J - 2 \delta_A^B J_I^{pm} I^J_{pn}  \bigr] v_{BJ}.
\end{align}
...
\end{align}
Such functions are often called scalars.
...
\begin{align}\label{EOM}
    \begin{aligned}
    0 &= E^C_{:m} \\
    0 &= E^{C:A} C_{An}^{Bm} v_B + E^{C:Ap} \bigl[ C_{An}^{Bm} \delta_p^q - \delta_A^B \delta^q_n \delta^m_p  \bigr] v_{Bq} + E^{C:AI} \bigl[ C_{An}^{Bm} \delta_I^J - 2 \delta_A^B J_I^{pm} I^J_{pn}  \bigr] v_{BJ}\\
...
\end{aligned}
\end{align}
These maps define curves on $M$. They track the path of specific spatial point $s \in \Sigma$ through spacetime, that defined by the family of embeddings. The situation can be seen in figure \ref{Slicing}.
...
\end{tikzpicture} 
\caption[Slicing of Spacetime into 3-dimensional Slices.]{Slicing of a $3$-dimensional spacetime into $2$-dimensional slices; the figure shows the embedded hypersurfaces $\Sigma_{\lambda_1}$ and $\Sigma_{\lambda_2}$ and further displays the induced curve through $s \in \Sigma$, $\phi_s$ and its tangential vector $\frac{\partial}{\partial t}\big \vert _{\phi_s(\lambda_2)}$ at the spacetime point $\phi_s(\lambda_2)$.}\label{Slicing}
\end{figure}
...
The Whitney sum then carries essentially the same information as the product of the two bundles, but as a bundle over $M$ not over $M \times M$.
When both $E$ and $F$ are vector bundles we can also give their Whitney sum the structure of a vector bundle. The bundle $E \oplus F$ is then exactly obtained as the fiber wise direct sum of $E$ and $F$.  
\end{remark}
...
Using the fact that all isomorphisms in the previous construction are isomorphisms of vector bundles and thus preserve the Whitney sum decomposition we find that also the spacetime tangent bundle then necessarily decomposes into a Whitney sum. In the following, we write
\begin{align}
...
\end{align}
The two remaining commutators, in general, contain additional contributions. As these additional contributions obviously depend on the specific form of the field dependency of $e_0$, we illustrate their appearance for the case of canonical GR. With slight modifications in the computations this will then of course also cover the case where for a given field theory the observer definition and thereby the vertical vector field $e_0$ is constructed in analogy to the standard GR case, but with an effective metric that is constructed from the field instead of the fundamental metric field in GR, as it is, for instance, the case for the observer definition in \cite{2018PhRvD..97h4036D}, \cite{2011PhRvD..83d4047R}, and \cite{Rivera}.
...
\section{Constrained Hamiltonian Dynamics}\label{ConstrainedDyn}
...
\begin{align}
     G_{t_0} := G \big \vert _{\Sigma_{t_0}},
\end{align}
...
\end{align}
where we used the priorly derived coordinate expression in charts adapted to the slicing for the last equality. Similarly we define the time derivative for the restriction of sections to the hypersurface as $\dot{G}_{t_0}:= \dot{G} \big \vert_{\Sigma_{t_0}}$, i.e., the time derivative of restrictions is the restriction of the corresponding time derivative.

The first step in our development of a Hamiltonian formulation of the given Lagrangian field theory consists of re-expressing the Lagrangian $\mathcal{L}(v_A,v_{Ap})$ not in terms of spacetime 1-jets of a given field but in terms of spatial jets, i.e., 3-dimensional 1-jets over $\Sigma_{t_0}$ of the field and its time derivative.
...
$(\mathcal{V}F)_{t_0}$, the restriction of the vertical subbundle $\mathcal{V}F \subset TF$ to the embedded hypersurface. We will denote these elements by $(\dot{G}_{t_0})_p$, where the subscript $p\in \Sigma_{t_0}$ labels the base point on the embedded hypersurface to whose fiber the element belongs.

Recall that the vertical subbundle is obtained from $TF$ as fiber wise kernel of the vector bundle morphism $(\pi_F)_{\ast} : TF \rightarrow
TM$. Details can be found in \cite{1998physics...1019G}. 
We can think of the restriction of this vertical bundle to the embedded hypersurface as encoding the derivative contributions to a spacetime 1-jet $j^1_pG$ that are vertical to the embedded hypersurface.
...
\begin{align}
    \beta_{t_0}\bigl ( (x^{\mu},(G_{t_0})_A, \partial_m(G_{t_0})_A) \big \vert_p \bigr ) = (x^{\mu},(G_{t_0})_A, \partial_{\mu}(G_{t_0})_A, (\dot{G}_{t_0})_A) \big  \vert _p.
\end{align}
...
\begin{aligned}
    ...
    \mathcal{L}_{t_0}(j^1_pG_{t_0}, (\dot{G}_{t_0})_p) &:= i_{\Sigma_{t_0}}^{\ast} \bigl( \mathcal{L}\circ \beta_{t_0}^{-1}(j^1_pG_{t_0}, (\dot{G}_{t_0})_p)(\frac{\partial}{\partial t},-)\bigr). 
\end{aligned}
\end{align}
\end{definition}
...
where
\begin{align}
\pi_{(G_{t_0})_p}((\dot{H}_{t_0})_p) := \frac{\partial L_{t_0}(j_p^1G_{t_0}, (\dot{G}_{t_0})_p+\lambda \cdot (\dot{H}_{t_0})_p)}{\partial \lambda } \bigg \vert _{\lambda = 0}\mathrm{d}^3x.
\end{align}
...
Using this, we obtain the \textit{\textbf{time dependent Hamiltonian}} by:
...
\end{aligned}
\end{align}
Note that, just as the time dependent Lagrangian, the time-dependent Hamiltonian takes values in $\Lambda^3\Sigma_{t_0}$ and thus can be integrated once it is evaluated on a section of  $J^1(F_{t_0}) \oplus (\mathcal{V}F)_{t_0}^{\dagger}$.
...
Furthermore, it is worth mentioning that here in our approach, the Legendre transform of the time-dependent Lagrangian is only taken w.r.t.\ the time derivative coordinates $\dot{v}_A$, in contrast to multi-momenta approaches, such a De Donder-Weyl theory (cf. \cite{deDonder}, \cite{Weyl}) where one defines conjugate momenta for all spacetime derivative coordinates of the field, not only for the time derivatives. This is for instance also discussed in \cite{1998physics...1019G}. 
...
Our next step lies in the construction of \textit{\textbf{Poisson brackets}} to encode dynamical information in the Hamiltonian formulation. 
...
Our main goal in the following section will be to translate the PDE system (\ref{DiffeoEqn}) that is obtained from the diffeomorphism equivariance of the Lagrangian into a language that allows us to use the resulting equations in the formulation of Hamiltonian dynamics. To that end, we choose adapted coordinates on $J^1F$ that additionally satisfy $x^0 = t$ and hence, we find $\frac{\partial}{\partial t} = \frac{\partial}{\partial x^0}$ and $\dot{v}_A = v_{A0}$. 
...
\end{align}
where any additional terms vanish as they are prolongations of (\ref{ConstrPi}) once we reinsert the definition of the momenta in terms of $L_{t_0}$. We now take equation (\ref{ConstrPi}), reexpress $\pi^A = L_{t_0}^{:A0}$, act with $\frac{\partial}{\partial v_A}C_{Am}^{D0}v_D$ and interchange the free indices $m$ and $n$ to get
...
Continuing the Dirac procedure, we have to add also these in Dirac's terminology \textit{\textbf{secondary constraints}} with multiplier functions $N=N(x^{\alpha})$ and $N^{\mu}= N^{\mu}(x^{\alpha})$ to the Hamiltonian. Absorbing the original Hamiltonian $H_{t_0}$ into the arbitrary multiplier $N$, we get
\begin{align}
\begin{aligned}
...
Hence, the bracket between two diffeomorphism constraints yields again a diffeomorphism constraint. 
...
Besides, we observe that the evolution generated by the secondary constraints satisfies the same algebra as the deformation of Hypersurface quantities displayed in (\ref{Alg}). This is something that we could have expected as in the context of Dirac's treatment of constrained Hamiltonians the constraints are said to generate gauge transformations (cf. \cite{1995JPhA...28.4431S}), which obviously in the case of diffeomorphism invariant field theory precisely correspond to diffeomorphisms. 
...
Although the ideas underlying the causal compatibility between matter and gravitational dynamics presented in the referenced work is unarguably of fundamental importance\footnote{We will, in fact, return to precisely these requirements in the following section.}, the interpretation that this constitutes the main ingredient of their framework solely from the appearance of parts of the principal polynomial ---already here the authors of \cite{2018PhRvD..97h4036D} remark that this appearance can, in fact, be traced back to the used observer frame--- in one of the commutation relations of the hypersurface deformation algebra does not seem compelling.
With our developments in mind, we would argue that also in their framework, the requirement that the canonical constraint algebra of gravity represents the algebra of hypersurface deformations, in fact, imposes the condition of diffeomorphism invariant dynamics on the theory of gravity.
