...
\end{align}
Abstractly, the PDE (\ref{DiffeoEqn}) can be defined as a submanifold of the first-order jet bundle $J^1(J^2F \oplus \Lambda^4M)$ with adapted coordinates
\begin{align}
(x^m,v_A,v_{Am},v_{AI},l,l^{m},l^{A},l^{Am},l^{AI}).
\end{align}
Thus, we obtain the formal representation of PDE (\ref{DiffeoEqn}) by replacing the bundle map $L$ and the derivatives $L_{:m},L^{:A},...$ with fiber coordinates $l$ and derivative coordinates $l^m,l^A,...$. Hence in terms of formal PDE theory the equivariance equation reads:
...
\begin{align}
    \sum_{i=0}^{k-2} (V_{\text{mat}})_i = \sum _{i=0}^{k-2}(V_{\text{grav}})_i + \mathcal{O}(k-2),
\end{align}
...
\begin{algorithm}[hbt!]
\SetAlgoLined
\KwData{Matter theory: $\mathcal{L}_{\text{mat}} : F_{\text{grav}} \oplus J^1F_{\text{mat}} \rightarrow \Lambda^4M$, expansion order: $k \in \mathbb{N}$, flat Lorentz invariant expansion point: $p_0 \in F_{\text{grav}}$}
...
Solve $\sum_{i=0}^{k-2} (V_{\text{mat}})_i = \sum _{i=0}^{k-2}(V_{\text{grav}})_i + \mathcal{O}(k-2)$ w.r.t. the remaining undetermined constants in $L_{grav,per}$.
