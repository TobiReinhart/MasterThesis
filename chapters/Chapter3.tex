...
\end{align}
This is the so called \textit{\textbf{Klein-Gordon}} Lagrangian. It is important to observe that this really describes the most comprehensive quadratic, first-order Lagrangian for a scalar field. 
In particular, note that specifying such a Lagrangian forces us to prescribe $10$ spacetime functions $B^{ab}$ that constitute the components of a $T_2^0M$ tensor field ---due to the symmetry $\phi_a \phi_b = \phi_{(a} \phi_{b)}$ we only need $10$ instead of $16$ such functions. These functions tell us how the product $\phi_a\cdot \phi_b$ has to be contracted. Furthermore, one needs an additional scalar density of weight one $\omega$ that ensures that the Lagrangian really defines a volume-form valued bundle map and thus provides us with a well-defined action functional.
...
Using the dual coordinates $(x^m,v^A)$ on $F_{\text{GR}}^{\ast}$ we define $v^A_{\flat} := v^A \circ \flat$. Thus, applying $v^A_{\flat}$ on any metric exactly yields the components of its inverse. Using this we get the explicit expression for $R$ understood as a map on $J^2F$ as
\begin{align}
...
\end{align}
In order to compute the expansion of $v^A_{\flat}$ one uses that $-1/2 \cdot v_{\flat}^AC_{An}^{Bm}v_B = v_{\flat}^A J_A^{nb}I^B_{bm}v_B = \delta^m_n$. Deriving this equation w.r.t.\ $v_B$ and performing some simplifications one thus finds the expression
\begin{align}
...
\end{align}
With these results computing, the expansion of $L_{\text{EH}}$ is straight forward, yet quite laborious.
...
\begin{align}
    J = d H.
\end{align}
From considerations involving the Lorentz force one similarly obtains that the magnetic analog to $J$ the field strength $F$ which plays the role of a 4-dimensional magnetic flux current density, and should thus be described by a 2 form $F \in \Gamma(\Lambda^2M)$. Its integral over a 2 dimensional submanifold $C \subset M$ describes the total magnetic flux ---the total magnetic flux flowing through $C$ and the total magnetic flux current\footnote{Note that by this interpretation rewriting $F$ in terms of a 3+1 split of spacetime and the usual $E$ and $B$ field the magnetic flux is as usual given by $B$ whereas $E$ is now interpreted as the corresponding magnetic flux current (see \cite{2006physics..10221H}).} flowing through the boundary of $C$. The corresponding conversation law, deduced in similar fashion as before then reads $dF =0$. Thus we infer, using again de Rham's first theorem, the existence of a 1 form $A \in \Gamma(\Lambda^1M)$ that satisfies
\begin{align}
    F = d A.
\end{align}
...
\end{align}
Note that in absence of a metric the Hodge star operator requires the choice of a volume-form (or equivalently a scalar density of weight 1) $\omega_G$. One possible such choice is for instance given by $\omega_G = \epsilon^{abcd}G_{abcd}$ which obviously only works if this expression is nowhere vanishing.
...
The other possibility is given when the representative of the equivalence class takes the general form $v_{ijkl}$, where $i<j$ or $(i=j) \land (k<l)$. The equivalence class then consists of the $8$ elements
\begin{align}
...
Next, we choose the $\eta$-induced expansion point $(N_{AI}) = (x_0^m, N_A, 0,0)$ as
\begin{align}
N_A := 2 \eta_{c[a} \eta_{b]d} - \epsilon_{abcd}.
\end{align}
Note that, assuming the density is taken such that $\omega_G(N_A) = \text{const} \neq 0$, when inserting $N_A$ into $\mathcal{L}_{\text{GLED}}$, one really obtains the Lagrangian of standard Maxwell Electrodynamics on a flat Minkowskian background
\begin{align}
    \mathcal{L}_{\text{GLED}}\bigl(N_{A},A_m,A_{mp}\bigr ) = 2 \omega_G(N_A) \eta^{ac}\eta^{bd}F_{ab}F_{cd} \mathrm{d}^4x.
\end{align}
Thus, expanding the to-be-constructed Lagrangian of area metric gravity around $(N_{AI})$ serves the purpose of interpreting the result as perturbative theory around a flat spacetime. We included $\epsilon_{abcd}$ in the definition of $N_A$ to ensure that the possible density choice $\omega_G = \epsilon^{abcd}G_{abcd}$ does not vanish when evaluated at $N_A$. 

We want to construct the perturbative area metric gravity theory up to order $k=3$. Following algorithm \ref{Algo2}, we proceed by computing the coordinate expression of the Lie derivative of $G_{abcd}$ to read off the vertical coefficient tensor $C_{An}^{Bm}$:
\begin{align}\label{LieArea}
    \mathcal{L}_{\xi} G_{abcd} = \partial_m G_{abcd} \cdot \xi^m + \left (4 \delta_n^{[e\vert} \delta_{[\mathnode{a}}^m \delta_{b]}^{\vert \mathnode{f} ]} \delta_{[c}^{[g} \delta_{\mathnode{d}]}^{\mathnode{h}]} \right ) \cdot G_{efgh} \partial_m \xi^n.
\end{align}
...
\begin{align}\label{LArea}
    ...
    + \mathcal{O}(4),
\end{align}
where we introduced the coordinate deviation from the expansion point
\begin{align}
...
\begin{aligned}
    \widetilde{\mathcal{P}}_{\text{GLED}} = \omega \cdot \mathcal{P}_{\text{GLED}} = 
...
\end{aligned}
\end{align}
Comparing this expression with the previously obtained area metric gravity polynomial (\ref{AreaPoly}), we find that multiplying the GLED polynomial with a density with constants $b_1 = A -C$ and $b_2 = \frac{1}{13}$, the two polynomials are products of the same factors and therefore, in particular, describe the same vanishing set in $\mathcal{O}(2)$. Thus, also for the case of area metric gravity requiring causal compatibility with the GLED matter theory in addition to diffeomorphism invariance at least perturbatively yields no further conditions for the parameters of $\mathcal{L}_{\text{Area,per}}$.
...
\begin{multline}
    ...
    + \mathcal{O}(2).
\end{multline}
Perturbatively, this polynomial describes the same vanishing set as (\ref{GenDif}). However, we now see see that we can, in fact, specify the remaining constant $b$ at wish just by multiplying with an appropriate density and hence, in particular, without changing the vanishing set. 
